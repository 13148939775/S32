...
KMeans::KMeans(const int dimNum, const int clusterNum, const int maxIterCnt)
{
	m_dimNum = dimNum;
	...
	for (int i = 0; i < m_clusterNum; i++) {
		...
		memset(m_means[i], 0, sizeof(double) * m_dimNum);  // 初始化的重心数值
	}

	m_initMode = InitRandom;
	m_maxIterNum = maxIterCnt;
	m_endError = 0.001;
}
...
{
	// HHT_LOG(INFO) << "Cluster ";
	// Check the sample file
	std::ifstream sampleFile(sampleFileName, ios_base::binary);
	assert(sampleFile);
