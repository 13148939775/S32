...
struct TaskLooper::Impl {
    ...
    typedef std::priority_queue<DealyTask, std::vector<DealyTask>,
                                std::greater<DealyTask> >
        DelayTaskQueue;
    typedef PriorityQueue<Task> ActionTaskQueue;
...
TaskLooper::TaskLooper(int32_t timeoutMs) : mImpl(new Impl()) {
...
void TaskLooper::postDelayTask(Task task, int32_t timeoutMs,
                                    int32_t priority) {
...
void TaskLooper::runOnce(int32_t timeoutMs) {
    ...
    Impl::TimePoint now = Impl::Clock::now();
    {
        ...
        while (!mImpl->mDelayTasks.empty() &&
               mImpl->mDelayTasks.top().actTime < now + MIN_RANGE) {
