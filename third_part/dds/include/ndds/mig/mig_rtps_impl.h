...
/*
 * @(#)mig_rtps_impl.h    generated by: makeheader    Wed Mar  9 22:30:14 2022
 *
...
/* --------------------------------------------------------- */
/*
 * Return the maximum length of a sample for a given channel. Useful to
 * protect the never encoding channels from some tampered messages that
 * force big memory allocations
 */
#define MIGRtpsObjectId_getMaxSampleLength(objectId__, unprotectedMaxLength__) \
    ((MIGRtpsObjectId_isNeverEncodingWriter(objectId__)                \
             || MIGRtpsObjectId_isNeverEncodingReader(objectId__))     \
                    ? (unprotectedMaxLength__)                         \
                    : RTI_CDR_MAX_SERIALIZED_SIZE)
...
/* --------------------------------------------------------- */
  #define MIGRtpsBitmap_isMalformed(me__) \
    (((me__)->_bitCount > MIG_RTPS_BITMAP_SIZE_MAX) || ((me__)->_bitCount < 0))
/* --------------------------------------------------------- */
  #define MIGRtpsBitmap_isDistanceValid(me__, distance__) \
        ((distance__ ) >= 0 && (distance__) < (me__)->_bitCount)
/* --------------------------------------------------------- */
