...
struct RTIXCdrTypePluginProgramContext {
    ...
    /* 
     * Indicates if the Interpreter functions must log errors that are are non 
     * expected space errors.
     *
     * For example:
     * - Failures due to range checking
     * - Failures getting a member value in a data sample
     * - Failures serializing strinsg and sequences with a length greater than
     * the maximum allowed.
     * - etc.
     */
     ...
     * XCDR stream.
     * 
     * The definition of expectedSpace error is different for serializationn and 
     * deserialization:
     * 
     * - On serialization, we treat all the errors due to running out of space
     * in the XCDR stream as expected. This is done because with bacthing we use 
