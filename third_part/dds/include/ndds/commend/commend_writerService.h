...
/*
 * @(#)commend_writerService.h    generated by: makeheader    Wed Mar  9 22:30:16 2022
 *
...
#define COMMENDFilterStatus_clear(me__) \
{ \
    RTI_UINT32 i__; \
    (me__)->status = 0; \
    (me__)->state = COMMEND_FILTER_ALL_INDEX_INVALID; \
    if ((me__)->_extended) { \
        (me__)->_extended[0].state = 0; \
        for (i__ = 1; i__ < (me__)->_extended[0].status; ++i__) { \
            (me__)->_extended[i__].status = 0; \
            (me__)->_extended[i__].state = COMMEND_FILTER_ALL_INDEX_INVALID; \
        } \
    } \
}

#define COMMENDFilterStatus_clearExtension(me__, start_index__) \
{ \
    RTI_UINT32 i__; \
    for (i__ = start_index__; i__ < (me__)->_extended[0].status; ++i__) {\
        (me__)->_extended[i__].status = 0; \
        (me__)->_extended[i__].state = COMMEND_FILTER_ALL_INDEX_INVALID;\
    }\
}

#define COMMENDFilterStatus_clearSampleStatus(me__) \
{ \
    RTI_UINT32 i__; \
    (me__)->status = 0; \
    if ((me__)->_extended) { \
...
#define COMMENDFilterStatus_droppedSampleStatus(me__) \
{ \
    RTI_UINT32 i__; \
    (me__)->status = 0xffffffff; \
    if ((me__)->_extended) { \
        for (i__ = 1; i__ < (me__)->_extended[0].status; ++i__) { \
            (me__)->_extended[i__].status = 0xffffffff; \
        } \
    } \
}
