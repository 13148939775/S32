...
static size_t sequence_length(const SequenceType &sequence) {
    return sequence.size();
}

inline
size_t sequence_length(const std::string &sequence) {
    return sequence.empty() ? 1 : strlen(sequence.c_str()) + 1;
}

inline
size_t sequence_length(const std::wstring &sequence) {
    return sequence.empty() ? 1 : RTIOsapiUtility_wcslen(sequence.c_str()) + 1;
}
...
struct sequence_helper {
    ...
    static RTIXCdrMemberValue get_value_pointer(
        ...
        void *)
    {
        // Important: this pointer must be const (CORE-11649). The interpreter
        // is guaranteed to call this function for read-only access.
