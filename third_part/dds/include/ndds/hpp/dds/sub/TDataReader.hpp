...
public:
     ...
     * -# Start with a call to `reader.select()` to return a Selector;
     * -# Follow with one or more Selector setters configure what data to read;
     * -# Finally call `read()` or `take()` to perform the operation.
     *
     ...
     * \endcode
     *
     * The selector also allows iterating over the samples one instance at a
     * time, using the next_instance() setter.
     *
    ...
    public:
         ...
         *
         * @param previous_handle The 'next smallest' instance with a value
        ...
        {
            impl_.next_instance(previous_handle);
            return *this;
        }
...
#ifndef RTI_NO_CXX11_SMART_PTR
    /**
     * @brief Create a DataReader for a dds::topic::ContentFilteredTopic with
     * a listener.
     *
     * This DataReader will only receive that data that mathes the Filter
     * associated with the ContentFilteredTopic.
