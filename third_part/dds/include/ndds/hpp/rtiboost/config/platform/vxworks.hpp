...
// Modified by RTI:
//
// Disable the following definitions when the user application is already using
// boost to avoid duplicate symbols. To ensure that these symbols are not
// redefined, always include boost headers before RTI headers. Alternatively
// compile with -DRTI_USE_BOOST.
#if !defined(BOOST_PLATFORM) && !defined(RTI_USE_BOOST)
...
#endif // !defined(BOOST_PLATFORM) && !defined(RTI_USE_BOOST)
