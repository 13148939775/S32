...
/*
 * @(#)transport_udp.h    generated by: makeheader    Wed Mar  9 22:30:07 2022
 *
 *		built from:	udp.ifc
 */
...
struct NDDS_Transport_UDP_Property_t {
    ...
    /*i
     * \brief If this property is set we will update the send
     * and receive statistics split into participant and endpoint discovery
     * by parsing the packets. We will update the discovery/user packets
     * based on the port number regardless of if this property is set.
     * The default is false for performance reasons and is set to true only 
     * for testing purposes.
     */
    RTIBool gather_detailed_statistics;
};
...
#define NDDS_TRANSPORT_UDP_DIFF_SERV_MAX_VALUE RTI_OSAPI_SOCKET_MAX_DIFF_SERV
...
struct NDDS_Transport_UDP_Statistics {
    ...
    /* 
     * Array of counters, one per DiffServ value. These counters will be
     * incremented based on the received DiffServ values of the UDP packet.
     * 
     * If received_priority_count[2] == 45 it means that we have received 45
     * packets with DiffServ value 2.
     * 
     * This field is populated only when gather_detailed_statistics is set
     * to 1.
     */
    RTI_UINT32
            received_priority_count[NDDS_TRANSPORT_UDP_DIFF_SERV_MAX_VALUE + 1];
};
